// "Our unfair advantage" — section intro (eyebrow + headline + lead). Comparison
// content to follow.

export default function UnfairAdvantage() {
  return (
    <section className="ua" aria-label="Our unfair advantage">
      <style>{`
        .ua {
          background: #000000;
          width: 100vw;
          margin-left: calc(50% - 50vw);
          margin-right: calc(50% - 50vw);
          padding: clamp(80px, 13vh, 180px) clamp(20px, 6vw, 88px);
          font-family: var(--font-sans), ui-sans-serif, system-ui, sans-serif;
        }
        .ua-head {
          max-width: 1000px;
          margin: 0 auto;
          text-align: center;
        }
        .ua-eyebrow {
          margin: 0;
          color: #ff791b;
          font-size: clamp(17px, 2vw, 24px);
          font-weight: 600;
          line-height: 1.17;
          letter-spacing: 0.009em;
        }
        .ua-title {
          margin: clamp(12px, 1.6vw, 20px) 0 0;
          color: #f5f5f7;
          font-size: clamp(34px, 5.4vw, 76px);
          font-weight: 600;
          line-height: 1.05;
          letter-spacing: -0.025em;
          text-wrap: balance;
        }
        .ua-lead {
          margin: clamp(18px, 2.2vw, 30px) auto 0;
          max-width: 720px;
          color: #86868b;
          font-size: clamp(17px, 1.5vw, 21px);
          font-weight: 500;
          line-height: 1.42;
          letter-spacing: -0.005em;
          text-wrap: balance;
        }
      `}</style>

      <div className="ua-head">
        <p className="ua-eyebrow" data-reveal>
          Our Unfair Advantage
        </p>
        <h2 className="ua-title" data-reveal style={{ ["--ri" as string]: 1 }}>
          A fancy design doesn&rsquo;t clean the air. The right filter does.
        </h2>
        <p className="ua-lead" data-reveal style={{ ["--ri" as string]: 2 }}>
          Most purifiers are built for small, sealed rooms and ideal lab
          conditions. Real spaces are bigger, busier, and dirtier. Here&rsquo;s
          the difference.
        </p>
      </div>
    </section>
  );
}
